...
A bit more complex: we need to estimate percentage changes from the day before. This is swiflty done with rollify. We can use this [function factory](https://adv-r.hadley.nz/function-factories.html) to build the function `roll_percent()`, which calculates percentage change over a column of a dataframe.
...
So, this is the clean dataframe that we use for plotting:
...
The numbers must appear above the arrow if the percentage change is positive, and, below, if it is negaitve. We achive this using `case_when()`: a vectorized ifelse statement. I use `case_when()` directly in the call to the aesthetic mapping.
...
***I would like to use this visualization on other variables of this datasets, and to comopare it with waterfall plots.***
